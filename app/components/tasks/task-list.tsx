...
import { windowTaskKey } from "@/lib/tasks/keys";
...
/** One-time migration of the legacy localStorage dismissal store: the old
    bare window ids ("voting") map onto occurrence keys for the CURRENT
    cycle (the only occurrence they could have meant), get POSTed to the DB
    store, and the legacy keys are deleted. The what's-next and
    Slack-clicked stores start fresh (weekly auto-expiry / one extra
    click). Delete this shim a cycle after it ships. */
...
export default function TaskList({
  ...
  useEffect(() => {
    migrateLegacyDismissals(activeCycleId, (keys) =>
