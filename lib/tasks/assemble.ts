...
export function assembleTasks(input: TaskInputs): Task[] {
  const tasks: Task[] = [];
  const survey = input.fieldSurvey;
  ...
  /* ── Start-here: baseline, field survey, first log ──────────────────── */
  ...
  if (survey && !input.surveyContributed) {
    tasks.push({
      defId: "survey_contribute",
      kind: "survey_contribute",
      instanceKey: surveyContributeTaskKey(survey.id),
      eyebrow: TASK_COPY.surveyContribute.eyebrow,
      title: survey.title,
      detail: TASK_COPY.surveyContribute.detail,
      href: `/survey/${survey.shareSlug}`,
      cta: TASK_COPY.surveyContribute.cta,
      secondaryHref: `/survey/${survey.shareSlug}/results`,
      secondaryCta: TASK_COPY.surveyContribute.secondaryCta,
      deadline: null,
      priority: PRIORITY.surveyContribute,
      tone: "teal",
      blocking: false,
      dismissible: false,
      done: false,
  ...
  /* ── Post-contribution survey share nudge ───────────────────────────── */
  if (survey && input.surveyContributed) {
    tasks.push({
      defId: "survey_share",
      kind: "survey_share",
      instanceKey: surveyShareTaskKey(survey.id),
      eyebrow: TASK_COPY.surveyShare.eyebrow,
      title: TASK_COPY.surveyShare.title,
      detail: TASK_COPY.surveyShare.detail,
      href: `/survey/${survey.shareSlug}`,
      cta: TASK_COPY.surveyShare.cta,
      secondaryHref: `/survey/${survey.shareSlug}/results`,
      secondaryCta: TASK_COPY.surveyShare.secondaryCta,
      deadline: null,
      priority: PRIORITY.surveyShare,
      tone: "default",
      blocking: false,
      dismissible: true,
      done: false,
