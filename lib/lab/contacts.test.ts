...
import { buildLabContactsTable, type LabContactRow } from "./contacts";
import { toCsv } from "@/lib/export/csv";

function row(overrides: Partial<LabContactRow> = {}): LabContactRow {
  return {
    first_name: "Ada",
    last_name: "Lovelace",
    preferred_name: null,
...
describe("buildLabContactsTable", () => {
  it("emits base + consent columns, with no status column", () => {
    const { columns } = buildLabContactsTable([]);
  ...
  it("serializes consent booleans as yes/no and null fields as empty cells", () => {
    const { columns, records } = buildLabContactsTable([
      row({
        first_name: "Ada",
        last_name: "Lovelace",
        phone_number: null,
