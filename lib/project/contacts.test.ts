...
import { buildProjectContactsTable, type ProjectContactRow } from "./contacts";
import { toCsv } from "@/lib/export/csv";

function row(overrides: Partial<ProjectContactRow> = {}): ProjectContactRow {
  return {
...
describe("buildProjectContactsTable", () => {
  it("emits every contact column in a stable order", () => {
    const { columns } = buildProjectContactsTable([]);
  ...
  it("sorts records by last name, then first name", () => {
    const { records } = buildProjectContactsTable([
      row({ first_name: "Grace", last_name: "Hopper" }),
      row({ first_name: "Ada", last_name: "Lovelace" }),
      row({ first_name: "Alan", last_name: "Hopper" }),
    ]);
    expect(
      (records as unknown as ProjectContactRow[]).map(
        (r) => `${r.first_name} ${r.last_name}`
      )
    ).toEqual(["Alan Hopper", "Grace Hopper", "Ada Lovelace"]);
  });

  it("serializes membership status and consent, with null fields as empty cells", () => {
    const { columns, records } = buildProjectContactsTable([
      row({
        first_name: "Ada",
        last_name: "Lovelace",
