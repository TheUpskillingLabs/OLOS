...
| `learning_logs` | Practice | The weekly ritual (replaces pulse_checks for new cycles). Weekly open-cycle rows are the v2 nine-item instrument (00087: stuck check, hours bucket, collab/progress/capability/energy ratings, contribution, learned, optionals); milestone/journal/org rows keep the v1 health check + reflection. Share flag on both. For `mode='org'` cycles also carries `work_summary`/`work_progress`/`work_blockers` (00069) — the member tier of the Leadership Log cascade |
