...
**`events.about` (migration `00094`):** the full Luma "About Event" text, plain, blank-line paragraphs — the opposite contract from the editorial fields: the sync overwrites it every tick, so the copy is maintained once, on Luma. The detail page renders it as prose under the lede **only while the event has no editorial `body`** — curated content takes precedence, and bespoke routes (the hackathon page) shadow the generic page entirely. Markdown emphasis/links are deliberately dropped (the View-on-Luma link carries anyone who wants the styled page).
