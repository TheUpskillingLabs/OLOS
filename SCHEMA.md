...
**`events.start_at` / `end_at`:** plain `TIMESTAMP` holding local wall time, rendered as written — the prototype's Luma-shaped date strings. `anchor = TRUE` marks the cycle's six anchor events; since `00092` the public `/events` page leads with the next three of them in a featured strip (the old ✦ glyph and anchor filter chip are gone). `location_type`: `in_person` · `virtual`.

**`events.kind` (migration `00092`):** the public events filter taxonomy — `Anchor` · `Workshop`, `NOT NULL DEFAULT 'Workshop'`, enforced by `events_kind_check`. The default is load-bearing: `syncLumaEvents()` treats `kind` as a local annotation and never sets it on insert, so without it every one of the several-times-daily Luma imports would land unclassified and match no filter chip. Backfilled from the `anchor` flag. `anchor` and `kind` therefore say the same thing twice — `anchor` is the machine-readable flag (featured strip, sync archive exemption), `kind` is the label on the card tag and the chip; flipping one does not move the other. The same migration corrected the four anchor rows whose dates had drifted from `lib/cycles/anchor-events.ts` / `cycle_events` (`scripts/ops/anchor-date-drift-2026-07-29.md`), and re-cast `hackathon-frame-sprint` as `civics-elections-hackathon` (Sat 2026-08-15 at American University, co-hosted) — that one event has a bespoke route at `app/(public)/events/civics-elections-hackathon/`, with the old path redirected in `next.config.ts`.

**`events.visibility` (migration `00093`):** `public` · `members`, NOT NULL DEFAULT `'public'`. Private Luma events (dry runs, check-ins — the admin-scoped API key returns them) sync as `members`: they render on the signed-in `/learning` agenda and their detail pages for signed-in visitors, and are invisible to the public `/events` page, anon PostgREST (tightened `events_public_read` + new `events_member_read`), and unfurlers. The sync owns the column and writes it only when Luma sends the field — absent means "leave as is", never "expose".

**`events.about` (migration `00094`):** the full Luma "About Event" text as markdown (`description_md` from Luma's per-event endpoint; the listing endpoint sends no description at all) — the opposite contract from the editorial fields: the sync overwrites it every tick, so the copy is maintained once, on Luma. The detail page renders it under the lede **only while the event has no editorial `body`** — curated content takes precedence, and bespoke routes shadow the generic page entirely. `lib/content/markdown.tsx` renders the subset Luma's editor can actually produce: paragraphs, one level of nested `- ` lists, `**bold**`, italics, links (markdown, bare, `mailto:`, and root-relative), `---` rules, bold-only lines promoted to subheadings (Luma gives authors no heading levels), and a **schedule block** — a run of `9:45`-style time rows, each with either inline text after a dash or a bullet list beneath, rendered as ruled rows with `Newcomer Track:`-style prefixes lifted into tags. Every rule degrades to plain paragraphs and lists when it does not match, so no author is penalised for writing normally. Parsing is pure and unit-tested (`markdown.test.ts`); rendering passes everything through React's escaping, never `dangerouslySetInnerHTML`.

**`events.location_address` / `meeting_url` (migration `00095`):** both Luma-owned, overwritten every tick. `locationOf()` used to collapse Luma's `geo_address_json` to a single string (first non-empty of `name`, `place_name`, `full_address`, `address`, `city`), so `location_name` was sometimes a venue and sometimes a street address depending on what Luma sent. Now `location_name` is the short display label and `location_address` the full postal address, which is what the detail page's "Where" link and the `.ics` `LOCATION` need — the map link is a keyless `maps.google.com/search` URL, the same kind Luma's own page uses. `meeting_url` was previously fetched from Luma and dropped on the floor, leaving virtual events with no join link; reads fall back to `location_name` when the address is null (legacy rows, until their next sync). The detail page also gives location a section of its own rather than only a `Kv` row in the rail, since `.detail-aside` is `display:none` below 1024px and a sidebar line is easy to miss either way.

**`events.sponsors` (migration `00095`):** **editorial, never written by the sync** — Luma renders sponsor logos on its own event page but does not expose them in the events API, so this is the one location-adjacent field the sync must not touch. Shape is `[{src, alt, bg?}]`, where `src` is a `/public` path or absolute URL and `bg: "dark"` marks knockout (white-on-transparent) art, which would otherwise vanish against the warm paper. Each logo renders in a fixed 180×96 `.lcard` tile with `object-fit: contain`: sponsors send wildly varied aspect ratios, and a tile both tidies the wall and lets `next/image` use `fill` instead of intrinsic dimensions we cannot know (no ratio guessing, no layout shift). `bg` is a deliberate human hint rather than pixel sampling — only whoever received the file knows what they have. No `/admin/content` editor yet; set it by SQL.

**`events.stats` (migration `00095`):** also **editorial, never synced**. Array of `[{n, label}]` for the numeral row ("12 / weeks per Build Cycle"). Editorial framing rather than anything Luma holds, and not inferable from prose. Both `stats` and `sponsors` are optional and omit their whole section when null, which is the general rule for this page: **every treatment gates on content and degrades to nothing**, so the same renderer serves a two-paragraph workshop and a full-day anchor event. `markdown.test.ts` pins that with a real short-workshop fixture asserting cards, panels, the schedule and the jump nav all stay dormant.

**Map embed:** the Location section renders a Google Maps Embed iframe when `NEXT_PUBLIC_GOOGLE_MAPS_EMBED_KEY` is set, and just the venue, address and an "Open in Maps" link when it is not — so local dev and previews never show a broken frame. The Embed API takes a plain address in `q`, so no coordinates are stored or geocoded.

**Luma sync (migration `00035`, `lib/integrations/luma.ts`):** Luma is the source of truth for ALL events. The scheduled sync (`/api/cron/sync-luma-events`, every 6h in production; manual trigger `POST /api/admin/events/sync` for admins) upserts by `api_id`, overwriting only Luma-owned fields (name, times, `location_type` / `location_name` / `location_address` / `meeting_url`, cover `img`, `luma_url`, `about`, `visibility`, and the initial fill-only `description`) — local annotations (`slug`, `kind`, `anchor`, `grad`, `cost`, `host`, `bring`, `body`, `gallery`) are never touched. `synced_at` set = Luma-managed row. Reconciliation archives published *future* rows missing from a successful fetch (cancelled/unlisted on Luma); past rows are kept as history, and `anchor = TRUE` rows are exempt (`00092` era fix) because their `anchor-0N` api_ids can never appear in a Luma listing.
...
**Member-view simulation (migration `00098`):** `simulation_sessions` is the audit trail for "View as" (`lib/auth/simulation.ts`) — an admin renders the member surfaces as a chosen participant, read-only, to debug what that person sees. Service-role only (RLS enabled, no policies), like `testers`. A row is written by `POST /api/admin/simulate` and stamped with `ended_at` by `GET /api/admin/simulate/exit`; the authorization is the signed, 1-hour cookie, not this table, so an un-stamped row (expired cookie, closed tab) grants nothing. Indexed both ways — by actor ("what has this admin been looking at") and by target ("who has viewed this member's account"). Guards live in app code, not SQL: the target must have signed in (`auth_user_id` set) and hold no `owner`/`admin`/`developer` row in `participant_roles`; on the prod project the actor must be an owner. Every mutation is blocked while the cookie is set (`proxy.ts` + `withAuth`), so nothing in the schema is written *through* a simulated identity.
...
| `follows` | Practice | The follow graph (00074): a member follows other members (`followee_participant_id`) or org pages (`page_type`+`page_id` = sector/workstream/lab, plus pod/project from 00076), polymorphic (exactly one target). Powers the "Following" feed — `profile_updates` are filtered to followed users + self, plus updates from followed/administered pages (00076). RLS: manage your own follows, read rows targeting you. Members auto-follow the pages they belong to (lab, pods, projects, workstreams) once on first dashboard load (`participants.page_follows_seeded`, 00076; `participants.lab_follow_seeded` from 00075 is the superseded lab-only precursor); "People you may know" (`lib/follows/suggestions.ts`) suggests podmates/labmates/cyclemates |
...
| `task_dismissals` | Practice | Per-member dismissals of dismissible dashboard task instances (00096). The `task_key` encodes the occurrence (`lib/tasks/keys.ts`), so recurring tasks re-fire under a new key — modeled on `nudge_dismissals` (00023), member-global instead of per-pod. Replaces the localStorage dismissal stores |
| `custom_tasks` | Practice | Admin-authored member tasks (00097, `/admin/tasks`): title/detail/link/CTA, optional cycle scope (NULL = program-global), optional visibility window (`ends_at` doubles as the displayed deadline), `pinned` (sorts under the gate) and `dismissible` flags. Merged into the queue by `lib/tasks/assemble.ts` as kind `custom`; retired via `archived_at`, never deleted |
| `testers` | Testing | Email-keyed tester grant — survives the tester's full self-reset |
| `simulation_sessions` | Testing | Audit trail for read-only "View as" — who viewed whose member dashboard, and when |
...
| `project_subscriptions` | Org Cycles | RETIRED (00077): project follow now lives in the unified `follows` graph (`page_type='project'`) — existing rows were back-filled into it; the table stays dormant for history. Was the IC ladder's self-serve entry point (00060) |
