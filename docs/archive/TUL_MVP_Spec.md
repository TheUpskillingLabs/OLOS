> **📁 ARCHIVED — historical record.** The founding product spec (spring 2026). The domain model (cycle → pod → project) largely shipped, but the stack here is superseded: there is no Python/FastAPI backend, no `pyjwt` JWT, and no `POST /auth/google` — OLOS is a Next.js monolith with Supabase-cookie auth and a granular `participant_permissions` model. The flat single-cohort formation described here is superseded by the HQ / Local-Lab model with per-lab pod formation, and the cycle status machine now has five states. Current truth: [docs/ARCHITECTURE.md](../ARCHITECTURE.md), [lib/auth/CLAUDE.md](../../lib/auth/CLAUDE.md), [SCHEMA.md](../../SCHEMA.md). See [docs/EVOLUTION.md](../EVOLUTION.md) for the full story of how the app got here.
