...
8. **Slack identity lives in a child table, not columns on `participants`:**
   `participant_slack_identities (participant_id, lab_id NULL, slack_user_id, state,
   linked_at)`, unique on `(participant_id, lab_id)`, resolved by `users.lookupByEmail` in
   the reconcile cron. One row per person today, with `lab_id` null. The table rather than
   three columns because a Slack user ID belongs to a workspace, so the day there is more
   than one workspace a person has more than one ID and neither is "the" one. A table now
   costs nothing; widening three columns later touches every caller.
   `participants.slack_username` stays and is display only, since the API will not accept
   it. Unresolvable people reach an `unmatched` state and are listed in admin for a
   hand-entered ID.
...
12. **Workspace topology is deliberately deferred, and the shape absorbs either answer.**
    One workspace today, the existing `theupskillinglabs`. A `slack_settings` row carries
    the name templates, default visibility and invite link with a **nullable `lab_id`**,
    null meaning "all labs", and exactly one row exists. Every function that reaches Slack
    takes a lab: `getSlackToken(labId)` and `getSlackInviteUrl(labId)` ignore the argument
    today and read `SLACK_BOT_TOKEN` and the invite URL from the environment. A second
    workspace later is then a row, a token, and one function body, not a refactor across
    every call site.

    What is threaded now and what is deferred is a deliberate split: parameters and table
    shapes now, because they are what spread through code; secret storage later, because
    encrypting a token in Postgres to serve a single workspace is cost with no benefit.
...
- A migration adds `cycles.lab_cycle_number` (unique with `lab_id`), `cycles.slack_code`
  ...
  `projects.slack_abbrev`, and the two new tables `slack_settings` and
  `participant_slack_identities`. Visibility config goes on `cycle_config`.
- `SLACK_BOT_TOKEN` stays a server-only environment variable, read through
  `getSlackToken(labId)` so the call sites never learn where it came from. It must never
  become a `NEXT_PUBLIC_` variable.
...
- **One workspace, or one per lab? Deliberately punted**, with decision 12 as the hedge.
  `docs/requirements/per-lab-configuration.md` anticipates per-lab Slack, and per-lab cycle
  numbering (decision 3) makes it matter: one workspace means every lab after the default
  needs a `slack_prefix`, while a workspace per lab frees `#c03` for all of them.

  What flips if it goes per-lab: the unique constraint on `slack_code` relaxes to
  `(lab_id, slack_code)`, prefixes and `metros.slack_prefix` become unnecessary, the token
  and invite link move from the environment into the settings row (encrypted, service-role
  only), each lab needs its own Slack app, plan tier and ten-app cap become per workspace,
  and `token_revoked` becomes a state a lab can enter on its own.

  The cost it would impose is worth recording now, because it is the argument against:
  a cross-lab HQ cycle (`00067_hq_open_cycle_sub_cohorts.sql`) could not have one channel,
  so some members would be asked to join two workspaces with two invite links and two
  identity rows. Slack Connect is the alternative and needs paid plans on both sides.

  Settled when a lab actually asks for its own workspace, and it should be settled before
  the first cross-lab cycle is scheduled either way.
