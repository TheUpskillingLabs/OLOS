...
| §1.6 | [ISSUE-W1-006](https://github.com/TheUpskillingLabs/OLOS/issues/44) | in review | inferno-gh (MJ) | branch `feature/magic-link-email` | Code complete. Spec's FastAPI JWT translated to `@supabase/ssr` session cookie + per-request role resolution; missing-participant case redirects to `/register` rather than 404. See [lib/auth/CLAUDE.md](../lib/auth/CLAUDE.md). Blocked on ops: Google Cloud OAuth client + Supabase Studio Google provider + redirect allow-list. |
| §1.7 | [ISSUE-W1-007](https://github.com/TheUpskillingLabs/OLOS/issues/45) | in review | inferno-gh (MJ) | branch `feature/magic-link-email` | Architectural deviation: invitation emails delivered via direct Resend HTTP API instead of Supabase SMTP relay (chosen for per-invite payload + branded template + send tracking). Custom-token flow, not Supabase magic-link OTP. See [lib/auth/CLAUDE.md](../lib/auth/CLAUDE.md). Blocked on Resend domain verification (SPF + DKIM). |
