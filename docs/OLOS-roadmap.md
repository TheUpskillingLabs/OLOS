...
| §1.7 | [ISSUE-W1-007](https://github.com/TheUpskillingLabs/OLOS/issues/45) | in review | inferno-gh (MJ) | merged to main via PR [#60](https://github.com/TheUpskillingLabs/OLOS/pull/60) | Code complete on main. **Ratified 2026-05-08** ([#64](https://github.com/TheUpskillingLabs/OLOS/issues/64)): invitation emails delivered via direct Resend HTTP API rather than Supabase SMTP relay — driven by free-tier rate limits (Supabase auth-email throttle would block bulk-invite §1.8). Custom-token flow, not Supabase magic-link OTP. See [lib/auth/CLAUDE.md](../lib/auth/CLAUDE.md). Blocked on Resend domain verification (SPF + DKIM). |
