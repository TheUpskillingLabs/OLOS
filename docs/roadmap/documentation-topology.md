...
| | |
|---|---|
| **Status** | Proposal — the mechanism is built and dry-run verified in this branch; the destination and access are decisions D12/D13 in [`next-sprint.md`](next-sprint.md) §7 |
| **Owner** | Owner (repository access) + docs owner (manifest and sweeps) |
| **Last verified** | 2026-09-25 against `main@226445a`; `TheUpskillingLabs/docs-archive` @ `8b33677` (2026-06-18) |
...
**The precedent.** The team already decided this once. `TheUpskillingLabs/docs-archive`
is a private repo created 2026-06-18 whose README describes exactly this archive and
lists twelve OLOS documents. **None of the twelve were ever pushed** — the repo holds
one file. A manual archive is an intention, not a process. The design below makes the
move automatic, one-way, and reviewable, and it reuses that repository.
...
Reuse **`TheUpskillingLabs/docs-archive`** (private; every team member gets read, the
docs owner and owner get write; the publish bot gets write). Renaming it to
`labs-knowledge` is optional — GitHub redirects the old name — and worth doing only if
`program/` and `governance/` are going to be used, because the name then stops
describing the content. Decision D12.

```
docs-archive/                      (or labs-knowledge/)
...
│   ├── curriculum/                ← the curriculum session's output (modules, scripts, templates)
...
1. **Confirm D12** — reuse `docs-archive`; decide on the rename.
...
| **Access model** | Whether `docs-archive` should hold anything partners or funders see (then a `public/` folder with its own rules) | no |
...
```markdown
# The Upskilling Labs — knowledge repository

Private. Everyone on the team can read; the program and governance folders are yours to
...
it there via a pull request. `olos/PUBLISHED.md` is the index. History: this repository
began 2026-06-18 as a manual archive (see git history for the original README); the
automated bridge is described in `olos/roadmap/documentation-topology.md`.
```
